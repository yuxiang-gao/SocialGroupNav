...
while 1:
    ...
    for k in range(len(self.states)):
        if k % 4 == 0 or k == len(self.states) - 1:
            center_rect = robot_positions[k]
            speed = (self.states[k - 1][0].px, self.states[k][0].py)
            ...
            for i in range(len(self.humans)):
                ...
                human = self.humans[i]
                des = human.get_goal_position()
                human_direction = (self.states[k - 1][1][i].px, self.states[k][1][i].py)
                pygame.draw.circle(screen, color, des, 10)
                pygame.draw.line(screen, BLACK, human_positions[j][i], add(d, (10* np.array(human_direction))), 3)
                if human_positions[j][i] != des:
                    human_positions[j][i] = add(human_positions[j][i], human_direction)     
