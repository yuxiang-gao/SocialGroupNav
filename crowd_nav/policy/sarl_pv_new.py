...
class SARLPV(MultiHumanRL, nn.Module):
    ...
    def forward(self, state):
        ...
        if self.with_global_state:
            ...
            if size[0] > 1 or scalar_num_people != size[1]:  # num_people[0]
                ...
                num_people = torch.tensor(num_people, dtype=torch.float32).repeat((1, size[1], self.last_mpl1))
                global_state = mlp1_output.view(size[0], size[1], -1)/num_people
            else:
    ...
    def forward_with_processing(self, state):
        ...
        if self.env.max_humans > 0:
            ob = np.concatenate((ob, np.zeros((1, self.env.max_humans - self.env.human_num, ob.shape[2]))), axis=1)
