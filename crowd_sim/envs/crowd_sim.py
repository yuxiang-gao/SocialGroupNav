...
from crowd_sim.envs.utils.render_teleop import render_teleop
...
class CrowdSim(gym.Env):
    ...
    def step(self, action, update=True):
        ...
        reaching_goal = (
            norm(end_position - np.array(self.robot.get_goal_position())) < self.robot.radius
        )
        ...
        if mode == "teleop":
            ...
            for k in range(len(self.states)):
                ...
                if k % 4 == 0 or k == len(self.states) - 1:
                    center = robot_positions[-1]
                    robot_render = pyg_render(center[0], center[1], 600, 600)
                    robot_render.run()
            ...
            for i in range(self.human_num + 1):
                ...
                if i == 0:
                    ...
                    arrows.extend(
                        [
                            patches.FancyArrowPatch(
                                *orientation[0], color=human_colors[i - 1], arrowstyle=arrow_style
                            )
                        ]
                    )
            ...
            if len(self.trajs) != 0:
                ...
                for traj in self.trajs:
                    human_future_position = [
                        [
                            tensor_to_joint_state(traj[step + 1][0]).human_states[i].position
            ...
            def update(frame_num):
                ...
                for i in range(self.human_num + 1):
                    ...
                    if i == 0:
                        arrows = [
                            patches.FancyArrowPatch(
                                *orientation[frame_num], color="black", arrowstyle=arrow_style
                            )
                        ]
                    else:
                        arrows.extend(
            ...
            def plot_value_heatmap():
                ...
                z = np.reshape(
                    z, (self.robot.policy.rotation_samples, self.robot.policy.speed_samples)
                )
            ...
            def print_matrix_A():
                ...
                for i in range(h):
                    print(
                        "{:<3}".format(i - 1)
                        + " ".join(["{:.3f}".format(self.As[global_step][i][j]) for j in range(w)])
                    )
                # with np.printoptions(precision=3, suppress=True):
                #     print('A is: ')
                #     print(self.As[global_step])
